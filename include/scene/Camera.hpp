...
public:
     ...
     * @brief
     */
    Camera(size_t width, size_t height, real distanceProj, const Direction<real, 3> &orientation) :
        _width(width),
        _height(height),
        _distanceProj(distanceProj),
        _orientation(orientation),
        _actualMoveSpeed{0, 0, 0},
        _actualRotSpeed(0, Direction<real, 3>{0,0,0}),
        _position{0, 0, 0},
        _rotSpeed(2.0f),
        _moveSpeed(2.0f),
        _zoomSpeed(2.0f),
     ...
     */
    void stop_turn() {
        _actualRotSpeed = Quaternion<real>(0, Direction<real, 3>{0,0,0});
    }
    ...
    void update() {
        ...
        _fieldOfView = generateFrustum();
        cout << "Position : " << _position << endl << "orientation : " << _orientation << endl;
    }

    Direction<real, 3> GetOrientation() const {
        return _orientation;
    }

    const real& GetFocalLength() const {
        return _focalLength;
    }
};
